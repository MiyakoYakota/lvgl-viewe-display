...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "Display custom image 'arty'");
    lvgl_port_lock(0);   // Lock LVGL

    lv_obj_t *img1 = lv_img_create(lv_scr_act()); // create image object
    lv_img_set_src(img1, &arty);                 // set your image source
    lv_obj_center(img1);                          // center it on screen

    // Zoom
    lv_img_set_zoom(img1, 256 * 2); // Zoom to 4 times (256 * 4 = 1024)
